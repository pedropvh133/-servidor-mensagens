/**
 * NOCTIS MESSENGER - SERVER V20.21 (MULTICAST SIGNALING)
 * ESTABILIZAÇÃO: Chamadas de Grupo e Promoção Instantânea.
 */
...
async function initB2() {
    ...
    } catch (e) { b2Status = `Erro: ${e.message} ❌`; }
}
initB2();
...
app.post('/user/update_pic', async (req, res) => {
    ...
    } else res.status(500).send('B2 Failed');
});
...
app.post('/group/promote', (req, res) => {
    ...
    if (group && group.admins.includes(adminUser)) {
        ...
        // SINAL DE CONTROLE: Notifica promoção instantânea
...
app.post('/call/signal', async (req, res) => {
    ...
    // MULTICAST: Se for um grupo, envia para todos os membros
...
// --- PÁGINA INICIAL ---
app.get('/', (req, res) => {
    res.send(`<h1>🛰️ NOCTIS Hybrid Turbo v20.21</h1><p>Status Firebase: ${firebaseStatus}</p><p>Backup: ${backupInfo}</p>`);
});

app.listen(port, () => console.log(`Noctis v20.21 pronto.`));
