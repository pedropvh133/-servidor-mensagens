/**
 * NOCTIS MESSENGER - SERVER V20.20 (ROBUSTEZ TOTAL)
 * ESTABILIZAÇÃO: Correção 404 em Fotos e Restauração de Gestão de Grupo.
 */
...
app.post('/user/update_pic', async (req, res) => {
    ...
    console.log(`B2: Iniciando upload para ${username}`);
    const b2Url = await uploadToB2(profilePic, `profile_${username}_${Date.now()}`);
    if (b2Url) {
        let user = users.find(u => u.username === username);
        if (!user && db) {
            // Se não está na RAM, tenta carregar do banco (Resolve 404)
...
app.post('/group/update_settings', (req, res) => {
...
app.get('/', (req, res) => {
    res.send(`
        <body style="background: #0B0E14; color: white; font-family: sans-serif; padding: 40px;">
            <h1>🛰️ NOCTIS Hybrid Server v20.20</h1>
...
app.listen(port, () => console.log(`Noctis v20.20 pronto.`));
